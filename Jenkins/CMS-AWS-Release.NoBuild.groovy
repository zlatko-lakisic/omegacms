    ...
    if (rollbackException == null) {
        try {
            dir(projectDirectory) {
                ...
                try {
                    bat "aws s3 sync \"${baseArtifactDirectory}\\admin.awslambda\\assets.${params.version}\" s3://${omegaAdminAssetsS3BucketName}"
                } catch (Exception e) {
    ...
    if (rollbackException == null) {
        try {
            dir(projectDirectory) {
                ...
                def lambdaArn = cmsTools.cms_getLambdaLayerArn("Administration", params.version)

                dir("MD.CMS.Administration.Core.AwsLambda") {

                    println "Creating IAM Role..."
                    awsTools.createIamRole(omegaAdminStackName, "..\\Assets\\aws\\rolepolicy.json")
                    awsTools.assignPolicyToIamRole(omegaAdminStackName, "arn:aws:iam::aws:policy/AWSLambda_FullAccess")
                    awsTools.assignPolicyToIamRole(omegaAdminStackName, "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole")
                    println "IAM Role created!"

                    println "Creating Environment Variables..."
                    def templateParameters = []
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "ProductLayer", lambdaArn, true)
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "BasePluginsLayer", "", true)
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "StageName", "admin", true)
                    ...
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "MDCMSAdministrationCorePluginsDirectory", "/opt/AwsLambdaPlugins/", true)
                    ...
                    def publish_opts = [
                        ...
                        "lambdaFunctionHandler": "MD.CMS.AwsLambda.Container.Core::MD.CMS.AwsLambda.Container.Core.LambdaEntryPoint::FunctionHandlerAsync",
                        "lambdaFunctionMemorySize": 512,
                        "lambdaFunctionTimeout": 30,
                        "templateParameters": templateParameters,
                        "configFile": "aws-lambda-tools-defaults.json",
                        "templateFile": "admin-serverless.template",
                        "package": "\"${baseArtifactDirectory}\\container.awslambda\\container.awslambda.1.0.0\""
                    ]
                    println "Lambda Options created!"

                    println "Creating Lambda Function and API Gateway..."
                    omegaAdminApiGatewayUrl = cmsTools.publishDotnetLambda(publish_opts, "https")
    ...
    if (rollbackException == null) {
        try {
            dir(projectDirectory) {
                ...
                def lambdaArn = cmsTools.cms_getLambdaLayerArn("WebApi", params.version)
                ...
                dir("MD.CMS.WebApi.Core.AwsLambda") {
                    ...
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "ProductLayer", lambdaArn, true)
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "BasePluginsLayer", "", true)
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "StageName", "ws", true)
                    ...
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "MDCMSWebApiCorePluginsDirectory", "/opt/AwsLambdaPlugins/", true)
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "BillingGroup", uniqueId, true)
                    ...
                    templateParameters = genericTools.appendObjectVariable(templateParameters, "MDToolsBaseDataAccessPluginsCoreBaseDataAccessPluginsDirectory", "/opt/AwsLambdaPlugins/", true)
                    ...
                    def opts = [
                        ...
                        "templateParameters": templateParameters,
                        "configFile": "aws-lambda-tools-defaults.json",
                        "templateFile": "webapi-serverless.template",
                        "package": "\"${baseArtifactDirectory}\\container.awslambda\\container.awslambda.1.0.0\""
                    ]

                    omegaWsApiGatewayUrl = cmsTools.publishDotnetLambda(opts, "wss")
                }
    ...
    if (rollbackException == null) {
        try {
            dir(projectDirectory) {
                ...
                def lambdaArn = cmsTools.cms_getLambdaLayerArn("WebApi-Sockets", params.version)
                ...
                dir("MD.CMS.WebApi.Sockets.Core.AwsLambda") {
                    ...
                    def opts = [
                        ...
                        "configFile": "aws-lambda-tools-defaults.json",
                        "templateParameters": templateParameters,
                        "templateFile": "websockets-serverless.template",
                        "package": "\"${baseArtifactDirectory}\\container.awslambda\\container.awslambda.1.0.0\""
                    ]

                    omegaWsSocketApiGatewayUrl = cmsTools.publishDotnetLambda(opts, "wss")
                }

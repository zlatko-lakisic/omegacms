...
var navigationMaster = {
    ...
    filter: {
        title: "Filters",
        link: helper.getUniqueFilename('filters.list'),
        members: []
    },
    mixin: {
        title: "Mixins",
...
function searchData(html) {
    ...
    var stripped = sanitizeHtml(html, {
        allowedTags: [],
        allowedAttributes: []
    });
 ...
 * @param {array<object>} members.directives
 * @param {array<object>} members.services
...
function buildNav(members) {
    ...
    if (members.directives.length) {
        ...
        members.directives.forEach(function (d) {
            if (!hasOwnProp.call(seen, d.longname)) {

                nav.directive.members.push(linkto(d.longname, d.longname.replace("module:", "")));
            }
    ...
    if (members.services.length) {
        ...
        members.services.forEach(function (s) {
            if (!hasOwnProp.call(seen, s.longname)) {

                nav.service.members.push(linkto(s.longname, s.longname.replace("module:", "")));
            }
            seen[s.longname] = true;
        });

    }

    if (members.filters.length) {

        members.filters.forEach(function (f) {
            if (!hasOwnProp.call(seen, f.longname)) {

                nav.filter.members.push(linkto(f.longname, f.longname.replace("module:", "")));
            }
            seen[f.longname] = true;
        });
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    data().each(function (doclet) {
        ...
        if (url.indexOf('global.html#md-cms-autocomplete') >= 0) {
            throw {};
        }
    ...
    for (var i = members.classes.length - 1; i >= 0; i--) {
        if (
            (members.classes[i].kind == 'class' && members.classes[i].scope == 'directive') ||
            (members.classes[i].kind == 'class' && members.classes[i].scope == 'filter') ||
            (members.classes[i].kind == 'class' && members.classes[i].scope == 'service')
        ) {
            members.classes[i].kind = members.classes[i].scope;
            members.classes[i].scope = 'global';
        }
    }
    ...
    members.filters = helper.find(data, { kind: 'filter' });

    for (var i = members.classes.length - 1; i >= 0; i--) {
        if (
            members.directives.filter(function (doc) { return doc.___id == members.classes[i].___id; }).length > 0 ||
            members.services.filter(function (doc) { return doc.___id == members.classes[i].___id; }).length ||
            members.filters.filter(function (doc) { return doc.___id == members.classes[i].___id; }).length > 0
        ) {
            members.classes.splice(i, 1);
        }
    }
    ...
    if (view.nav.directive && view.nav.directive.members.length) {
        generate('directive', view.nav.directive.title, [{
            kind: 'sectionIndex',
            contents: view.nav.directive
        }], navigationMaster.directive.link);
    }

    if (view.nav.filter && view.nav.filter.members.length) {
        generate('filter', view.nav.filter.title, [{
            kind: 'sectionIndex',
            contents: view.nav.filter
        }], navigationMaster.filter.link);
    }

    if (view.nav.service && view.nav.service.members.length) {
        generate('service', view.nav.service.title, [{
            kind: 'sectionIndex',
            contents: view.nav.service
    ...
    for (var longname in helper.longnameToUrl) {
        if (hasOwnProp.call(helper.longnameToUrl, longname)) {
            ...
            var myDirectives = helper.find(directives, {
                longname: longname
            });
            ...
            var myServices = helper.find(services, {
                longname: longname
            });
            ...
            var myFilters = helper.find(filters, {
                longname: longname
            });
            if (myFilters.length) {
                generate('filter', 'Filter: ' + myFilters[0].name, myFilters, helper.longnameToUrl[longname]);
            }

(function ()
{
    ...
    angular
        .module('app.core')
        .directive('hljs', ['$timeout', '$q', '$interpolate', hljsDirective]);
    ...
    function hljsDirective($timeout, $q, $interpolate)
    {
        return {
            restrict: 'EA',
            compile : ['tElement', 'tAttrs', function (tElement, tAttrs)
            {
                var code;
                ...
                return ['scope', 'iElement', 'iAttrs', function (scope, iElement, iAttrs)
                {
                    if ( iAttrs.code )
                    {
                    ...
                    $q.when(code).then(function (resultCode)
                    {
                        if (resultCode )
                        {
                            if ( shouldInterpolate )
                            {
                                code = $interpolate(resultCode)(scope);
                            }

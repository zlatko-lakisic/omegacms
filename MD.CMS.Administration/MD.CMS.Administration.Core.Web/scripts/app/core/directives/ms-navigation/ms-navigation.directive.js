(function () {
    ...
    angular
        .module('app.core').provider('msNavigationService', msNavigationServiceProvider)
        .controller('MsNavigationController', ['$scope', 'msNavigationService', '$templateCache', '$q', MsNavigationController])
        // Vertical
        .directive('msNavigation', ['$rootScope', '$timeout', '$mdSidenav', 'msNavigationService', msNavigationDirective])
        .controller('MsNavigationNodeController', ['$scope', '$element', '$rootScope', '$animate', '$state', 'msNavigationService', 'mdUiVisibilityService', MsNavigationNodeController])
        .directive('msNavigationNode', msNavigationNodeDirective)
        .directive('msNavigationItem', msNavigationItemDirective)
        //Horizontal
        .directive('msNavigationHorizontal', ['msNavigationService', msNavigationHorizontalDirective])
        .controller('MsNavigationHorizontalNodeController', ['$scope', '$element', '$rootScope', '$state', 'msNavigationService', 'mdUiVisibilityService', MsNavigationHorizontalNodeController])
        .directive('msNavigationHorizontalNode', msNavigationHorizontalNodeDirective)
        .directive('msNavigationHorizontalItem', ['$mdMedia', msNavigationHorizontalItemDirective]);
    ...
    function msNavigationServiceProvider() {
         ...
         * @param item
         */
        function saveItem(path, item, postLoad) {
         ...
         * @param path
         */
        function deleteItem(path, recursive) {
            if (!angular.isString(path)) {
                $log.error('path must be a string (eg. `dashboard.project`)');
                return;
            }
    ...
    /** @ngInject */
    function MsNavigationController($scope, msNavigationService, $templateCache, $q) {
        var vm = this;
        var navigation = [];
        // Data
        if ($scope.root) {
            navigation = msNavigationService.getNavigationObject($scope.root);
        }
        else {
            navigation = msNavigationService.getNavigationObject();
        }

        var navPromises = [];

        vm.template = 'navigation-nested.html';

        function allItemsLoaded(init) {
            if (init === undefined) {
                init = false;
            }

            if (!init) {
                vm.template = 'navigation-nested-reloaded.html';
            }
            if ($scope.root) {
        ...
        function getCallback(navigation) {
            for (var i = 0; i < navigation.length; i++) {
                if (navigation[i].callback !== undefined) {
                    navPromises.push(navigation[i].callback($scope, $q));
                }
                if (navigation[i].children !== undefined) {
                    getCallback(navigation[i].children);
                }
            }
        }

        $scope.$on('LoadMenuFromDirective', init);

        $scope.$emit('NavigationLoaded');
    ...
    function msNavigationDirective($rootScope, $timeout, $mdSidenav, msNavigationService) {
        return {
            ...
            templateUrl: 'scripts/app/core/directives/ms-navigation/templates/vertical.html',
            transclude: true,
    ...
    function displayNode(node, mdUiVisibilityService) {
        return node._path.split('.').filter(function (path) {
            return mdBusinessLogic.globals.loggedOnUser.ProfileTypes.filter(function (profile) {
                var isVisible = false;
                var visibleEntry = mdUiVisibilityService.get(path, 1, profile.Id);
                if (visibleEntry != null) {
                    isVisible = visibleEntry.visible == false;
                }

                if (angular.isDefined(node.hidden) && angular.isFunction(node.hidden)) {
                    isVisible = !node.hidden() && isVisible;
                }
    ...
    ///** @ngInject */
    function MsNavigationNodeController($scope, $element, $rootScope, $animate, $state, msNavigationService, mdUiVisibilityService) {
        var vm = this;
        ...
        function init() {
            ...
            $scope.$watch(function () { return mdBusinessLogic.globals.loggedOnUser.ProfileTypes; }, function () {
                vm.node.display = displayNode(vm.node, mdUiVisibilityService);
            });
         ...
         * Toggle collapsed
         */
        function toggleCollapsed(disabled) {
                ...
                return function (scope, iElement, iAttrs, MsNavigationNodeCtrl) {
                    ...
                    if (MsNavigationNodeCtrl.collapsable) {
                        iElement.on('click', function () {
                            MsNavigationNodeCtrl.toggleCollapsed($(this).hasClass("collapseDisabled"));
                        });
                        iElement.find('.md-mini-cms').on('click', MsNavigationNodeCtrl.toggleCollapsed);
                    }

                    // Cleanup
                    scope.$on('$destroy', function () {
                        iElement.off('click');
                        iElement.find('.md-mini-cms').off('click');
                    });
                };
            }
        };
    ...
    function msNavigationHorizontalDirective(msNavigationService) {
        return {
            ...
            controller: 'MsNavigationController as vm',
            templateUrl: 'scripts/app/core/directives/ms-navigation/templates/horizontal.html',
            transclude: true,
            compile: function (tElement) {
    ...
    /** @ngInject */
    function MsNavigationHorizontalNodeController($scope, $element, $rootScope, $state, msNavigationService, mdUiVisibilityService) {
        var vm = this;
    ...
    function msNavigationHorizontalNodeDirective() {
        return {
            ...
            compile: function (tElement) {
                ...
                return function (scope, iElement, iAttrs, MsNavigationHorizontalNodeCtrl) {
                    // Add custom classes

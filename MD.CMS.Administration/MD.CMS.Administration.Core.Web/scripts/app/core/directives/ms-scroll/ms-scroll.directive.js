(function ()
{
    ...
    angular
        ...
        .directive('msScroll', ['$timeout', 'msScrollConfig', 'msUtils', 'fuseConfig', msScrollDirective]);
        ...
        return {
            ...
            compile : function (tElement)
            {
                ...
                return function postLink(scope, iElement, iAttrs)
                {
                    ...
                    $timeout(function ()
                    {
                        perfectScrollbar = new PerfectScrollbar(iElement[0], options);
                    }, 0);
                    ...
                    {
                        if(perfectScrollbar != null){
                            perfectScrollbar.update(iElement[0]);
                            scope.$broadcast('ms-scroll-update-scrollbar', perfectScrollbar);
                        }
                    }
                    ...
                    scope.$on('$destroy', function ()
                    {
                        ...
                        if(perfectScrollbar != null){
                            ...
                            scope.$broadcast('ms-scroll-destroy-scrollbar');
                        }
                    });

                    iElement[0].addEventListener('ps-scroll-y', function () {
                        var _perfectScrollbar = angular.copy(perfectScrollbar);

                        _perfectScrollbar.currentScrollTop = angular.element(iElement[0]).scrollTop();
                        _perfectScrollbar.currentScrollLeft = angular.element(iElement[0]).scrollLeft();

                        scope.$broadcast('ms-scroll-event-scroll-y', _perfectScrollbar);
                    });

                    scope.$on('ms-scroll-get-info', function (event, data) {
                        scope.$broadcast('ms-scroll-event-info', perfectScrollbar);
                    });
                };
            }
        };

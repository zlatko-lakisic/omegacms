...
tinymce.PluginManager.add('cmsmedia', function (editor, url) {
    var urlPatterns = [
    ...
    function buildListItems(inputList, callback) {
        function appendItems(values, output) {
            ...
            tinymce.each(values, function (item) {
                ...
                gridItem.onPostRender = function () {
                    var valueString = dataToHtml({ source1: gridItem.value, width: gridItem.width, height: gridItem.height })
                    this.innerHtml("<div style='border:2px solid black;height:160px;position:relative;' class='cms-media-container'>" +
                                    "<video width='" + gridItem.width + "' height='" + gridItem.height + "' style='object-fit:contain;max-width:95%;max-height:95%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);'><source src='" + gridItem.value + "' type='" + gridItem.mime + "'></source></video>" +
    ...
    function addEvents() {
        $('.cms-media-container').click(function () {
            var self = this;
            $('.cms-media-selected').removeClass('cms-media-selected');
            $(self).addClass('cms-media-selected');
    ...
    function showDialog() {
        ...
        var videoListCtrl, videoList = editor.settings.cmsmedia.video_list;

        var refreshCallback = editor.settings.cmsmedia.refresh;
        ...
        function refreshList(items) {
            if (items !== undefined) {
                videoListCtrl.items().remove();
                videoListCtrl.add(buildListItems(items, selectVideo));
                videoListCtrl.renderNew();
                addEvents();
            }
        }

        function selectVideo(item) {
            win.find("#source1").value(item.value);
            win.find("#width")[0].value(item.width);
            win.find("#height")[0].value(item.height);
        }
        ...
        if (videoList) {
            videoListCtrl = {
                ...
                layout: 'grid',
                columns: 2,
                items: buildListItems(videoList, selectVideo),
            ...
            body: [
				{
				    ...
				    type: "form",
				    onShowTab: function () {
				        data = htmlToData(this.next().next().find('#embed').value());
				        this.fromJSON(data);
				    },
                    ...
                    items: [
                        {
                            ...
                            label: 'Search',
                            onsubmit: function (e) {
                                var callback = editor.settings.cmsmedia.search;
                                if (callback && typeof callback == 'function') {
                                    callback($(this.$el).val(), refreshList);
                                } else {
    ...
    editor.addShortcut('Meta+O', '', showDialog);

    editor.addButton('cmsmedia', {
        icon: 'media',

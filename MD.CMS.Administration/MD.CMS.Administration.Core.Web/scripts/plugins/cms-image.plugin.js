tinymce.PluginManager.add('cmsimage', function (editor) {
    ...
    function buildGridItems(inputList, callback) {
        function appendItems(values, output) {
            output = output || [];
            ...
            tinymce.each(values, function (item) {
                ...
                gridItem.onPostRender = function () {
                    this.innerHtml("<div class=\"cms-image-container\">" +
                                    "<img class=\"media\" src='" + gridItem.value + "' />" +
                                    "<p class=\"label-image\">" + gridItem.title + "</p>" +
                                    "</div>");
                }
    ...
    function createImageList(callback) {
        return function () {
            var imageList = editor.settings.cmsimage.image_list;
    ...
    function showDialog(imageList) {
        ...
        var width, height, imageGridCtrl, classListCtrl, imageDimensions = editor.settings.image_dimensions !== false;

        var refreshCallback = editor.settings.cmsimage.refresh;
        ...
        function selectImage(item) {
            win.find("#src").value(item.value);
            win.find("#alt").value(item.description);
            getImageSize(editor.documentBaseURI.toAbsolute(item.value),
                function (data) {
        ...
        function refreshList(items) {
            imageGridCtrl.items().remove();
            imageGridCtrl.add(buildGridItems(items, selectImage));
            imageGridCtrl.renderNew();
            addEvents();
        }

        function addEvents() {
            $('.cms-image-container').click(function () {
                var self = this;
                $('.cms-image-selected').removeClass('cms-image-selected');
                $(self).addClass('cms-image-selected');
        ...
        if (imageList) {
            imageGridCtrl = {
                ...
                layout: 'grid',
                columns: 2,
                items: buildGridItems(imageList, selectImage),
        ...
        // Advanced dialog shows general+advanced+cms tabs
                    ...
                    items: [
                        {
                            ...
                            label: 'Search',
                            onsubmit: function (e) {
                                var callback = editor.settings.cmsimage.search;
    ...
    editor.addShortcut('Meta+E', '', showDialog);

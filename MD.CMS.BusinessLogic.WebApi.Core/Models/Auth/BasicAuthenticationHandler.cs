...
namespace MD.CMS.BusinessLogic.WebApi.Core.Modeles.Auth
{
    public class BasicAuthenticationHandler : AuthenticationHandler<BasicAuthenticationOptions>
    {
        ...
        {
            if (!Request.Headers.ContainsKeyName(AuthorizationHeaderName))
            {
                //Authorization header not in request
                return AuthenticateResult.NoResult();
            }

            if (!AuthenticationHeaderValue.TryParse(Request.Headers.GetValue(AuthorizationHeaderName), out AuthenticationHeaderValue headerValue))
            {
